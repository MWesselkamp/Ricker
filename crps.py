...
class CRPS:
    ...
    def compute(self):
        ...
        if (self.ob is not torch.nan) and (not torch.isnan(self.fc).any()):
            ...
            self.crps = torch.sum(torch.tensor((self.__cdf_fc - self.__cdf_ob) ** 2) * self.__delta_fc)
            if self.__m == 1:
                self.fcrps = self.acrps = 'Not defined'
            else:
                self.fcrps = self.crps - torch.sum(
                    torch.tensor(((self.__cdf_fc * (1 - self.__cdf_fc)) / (self.__m - 1)) * self.__delta_fc))
                self.acrps = self.crps - torch.sum(torch.tensor((((1 - (self.__m / self.M)) * self.__cdf_fc * (
